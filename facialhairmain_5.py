...
# ---------------------------
# Assign har Labels from Ground Truth
# ---------------------------
...
for root, dirs, files in os.walk(faces_folder):
    for file in files:
        if file.endswith(".jpg"):  # Check if the file is a .jpg image
            image_path = os.path.join(root, file)  # Full image path
            folder_name = os.path.basename(root)  # Get the folder name as the label (e.g., person id)
...
def create_graphs(gen,imp):
    ...
    system_name = "Facial Hair  Score - 5 Landmarks"  # Replace with your system name
